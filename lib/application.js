...
Application.prototype.start = function(callback) {
  ...
  async.series([
    ...
    // Load all fields, types and create collections for types in Waterline.
    function(next) {
    ...
    function(next) {
      ...
      utils.extend(pranaAdapter, adapterMethods(self));
      ...
      self.waterline.initialize(config, function(error, models) {
        ...
        for (var collectionName in models.collections) {
          ...
          collection.type = self.types[schema.tableName];

          self.collections[schema.tableName] = collection;
        }
  ...
  if (settings.fields) {
    ...
    for (fieldName in settings.fields) {
      ...
      if (fieldSettings.type == 'reference' && fieldSettings.reference) {
        ...
        var field = this.fields[fieldSettings.type];
