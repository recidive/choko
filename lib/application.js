...
var Application = module.exports = function(mainApplication, hostname, settings) {
  ...
  var application = this.application = express();
  ...
  Object.keys(routers).map(function(routerName) {
    application.use(routers[routerName]);
  });
...
Application.prototype.start = function(callback) {
  ...
  staticRouter.use(express.static(this.settings.applicationDir + '/public'));
  staticRouter.use(express.static(this.settings.baseDir + '/applications/default/public'));
  ...
  this.loadExtensions(extensionsPaths, function(error, extensions) {
    ...
    async.each(Object.keys(extensions), function(extensionName, next) {
      ...
      fs.exists(filesPath, function(exists) {
        if (exists) {
          self.routers.static.use(express.static(filesPath));
        }
        next();
      });
    }, callback);
