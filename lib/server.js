...
Server.prototype.addApplication = function(hostname, dir, callback) {
  ...
  Object.keys(applications).forEach(function(name) {
    ...
    environments.forEach(function(enviroment) {
      ...
      // Register file paths.
      settingFiles.push(filePath + 'json');
      localSettingFiles.push(filePath + 'local.json');
    });

    // Loop through defined file paths to load the ones that exist. 
    settingFiles.concat(localSettingFiles).forEach(function(filePath) {
