...
Server.prototype.addApplication = function(hostname, dir, callback) {
  var environments = [null, 'local', process.env.NODE_ENV || 'production'];
  ...
  var settings = {
    ...
    applicationDir: applications.main
  };

  // Loop through applications.
  Object.keys(applications).forEach(function (name) {
    // Loop through environments to find and load specific settings and add
    // each of the found items to the extension list.
    environments.forEach(function (enviroment) {
      var settings;
      var filePath = path.relative(__dirname, applications[name])
                   + '/settings.'
      ...
      try {
        ...
        // No problem if requre fails.
      } finally {
        environmentSettings.push(settings || {});
      }
    });
  });

  // Load and merge all the settings.
  _.merge.apply(null, [settings].concat(environmentSettings));
